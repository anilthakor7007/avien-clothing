...
const NewArrival = () => {
  ...
  const nextProducts = () => {
    setStartIndex((prevIndex) =>
      prevIndex + 4 >= newArrivalProducts.length ? 0 : prevIndex + 4
    );
  };

  const prevProducts = () => {
    setStartIndex((prevIndex) =>
      prevIndex === 0 ? newArrivalProducts.length - 4 : prevIndex -4
    );
  };
  ...
  return (
    <>
      <div className="title">
        <h1 className="font-bold text-2xl text-center pt-7">NEW ARRIVALS</h1>
      </div>
      <div className="relative w-full h-[600px] overflow-hidden pt-5">
        <div className="flex">
          {newArrivalProducts.slice(startIndex, startIndex + 4).map((product, index) => (
            <div
              key={index}
